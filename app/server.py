...
def save_as(body: dict):
    """
    Save a prediction to a specified path.
    Body: {"prediction": {...}, "output_path": "/abs/path/results.xlsx"}
    ...
    if not p.suffix.lower() in (".xlsx", ".xls"):
        raise HTTPException(400, "output_path must end in .xlsx")
    if not p.parent.exists():
        raise HTTPException(400, f"Directory does not exist: {p.parent}")
    out = save_result(pred, p)
