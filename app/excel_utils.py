...
def save_result(pred: dict, output_path: Path | None = None) -> str:
    """Append a prediction to the output Excel file. Returns the output path."""
    OUTPUT_PATH_USED = output_path or OUTPUT_PATH
    if OUTPUT_PATH_USED.exists():
        wb = openpyxl.load_workbook(str(OUTPUT_PATH_USED))
        ws = wb.active
    ...
    wb.save(str(OUTPUT_PATH_USED))
    return str(OUTPUT_PATH_USED)
